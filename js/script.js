const global = {
    ...
    api: {
        apikey: '988d7ec35faa87dc69a8eaf981977e2b',
...
async function search() {
    ...
    if (global.search.term != '' && global.search.term != null) {
         const movies = await searchAPIData();
          console.log(movies.total_results)
         if(movies.results.length === 0){         
            showAlert('No Results Found', 'error');
            return;
         } else {
            global.search.page = movies.page;
            global.search.totalPages = movies.total_pages;
            global.search.totalResults = movies.total_results;
...
async function searchAPIData () {
    ...
    const response = fetch(`${API_URL}search/${global.search.type}?api_key=${API_KEY}&language=en-US&query=${global.search.term}&page=${global.search.page}`);
